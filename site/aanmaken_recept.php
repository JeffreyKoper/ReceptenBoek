    ...
    if(isset($_POST["submitButton"])){
        if(!empty($_POST["titel"]) && !empty($_POST["beschrijving"])){ 
           if(strlen($_POST["titel"]) >= 3){
                ...
                $stmt = $conn->prepare("INSERT INTO Recepten (gerecht_naam, beschrijving, tijdsduur, menugang, moeilijksheidgraad, gebruiker_id)
                VALUES (:name, :beschrijving, :tijdsduur, :menugang, :moeilijksheidgraad, :id)");
